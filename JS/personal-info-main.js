...
window.initPersonalInfo = function () {
  if (config) {
    ...
    if (config.portfolio) {
      for (let i = 0; i < config.portfolio.length; i++) {
        if (i % 3 == 0) {
          $(".portfolio-section-main-container").append(
            '<div class="row"></div>'
          );
          row_i++;
        }
  ...
  if (
    $(window).scrollTop() >=
    $(".page-header").height() - $(".page-header .bg-primary").height()
  ) {
    $(".page-header .bg-primary").css(
      "cssText",
      "background-color:rgb(55 65 178)!important"
    );
  }
  ...
  function mailsend() {
    ...
    // }
    location =
      "mailto:" + config.email + "?subject=" + subject + "&body=" + content;
  }
